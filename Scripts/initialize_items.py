...
stims_full_path = [f for f in glob.glob(stim_path)]
stims_filename = [f.rsplit('/',1)[1] for f in stims_full_path]
...
beach_con = ['/beach/' in i for i in stims_full_path]
camp_con = ['/camp/' in i for i in stims_full_path]
incon = ['/incongruent/' in i for i in stims_full_path]
...
pd.DataFrame.to_csv(df_all_stims,os.path.join(exp_dir,'Protocol','stim_list.csv'))
pd.DataFrame.to_csv(counterbalance_df,os.path.join(exp_dir,'Protocol','counterbalance.csv'))